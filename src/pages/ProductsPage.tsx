...
export const ProductsPage = () => {
    const { data, isFetchingNextPage, isError, error, fetchNextPage} = useGetProducts(20)

    const observerRef = useCallback((node: HTMLDivElement) => {
        console.log('create observer')
        const observer = new IntersectionObserver(() => {
            console.log('IntersectionObserver run')
            fetchNextPage()
        })
    ...
    return (
        <main>
            <div className='grid gap-2'>
                ...
                {isFetchingNextPage ? <div className='flex justify-center items-center py-5'>
                    <Loader/>
                </div> : null}
                {isError ? <span className='bg-red-300'>{error.message}</span> : null}
                <div ref={observerRef}/>
            </div>
        </main>
    );
